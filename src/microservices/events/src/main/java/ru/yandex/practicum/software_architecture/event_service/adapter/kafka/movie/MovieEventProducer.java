...
public class MovieEventProducer {
    ...
    public EventResponse sendMessage(String message) {
        log.info("Попытка отправки сообщения в топик {} : {}", KafkaTopicConfig.MOVIE_TOPIC_NAME, message);
        CompletableFuture<SendResult<String, String>> future =
                kafkaTemplate.send(KafkaTopicConfig.MOVIE_TOPIC_NAME, message);

        try {
            SendResult<String, String> result = future.get();

            log.info("Отправлено сообщение {} с offset [{}] в топик {}",
                    message, result.getRecordMetadata().offset(), KafkaTopicConfig.MOVIE_TOPIC_NAME);
